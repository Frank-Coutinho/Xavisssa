...
namespace Xavissa.Backend.Controllers
{
    ...
    public class SalesController : ControllerBase
    {
        ...
        [HttpGet("summary")]
        public async Task<ActionResult<SalesReportViewModel>> GetSummary(DateTime? start = null, DateTime? end = null)
        {
        ...
        public async Task<ActionResult<List<Sale>>> GetByPaymentMethod(string paymentMethod)
        {
            ...
            var sales = await _db.Sales
                .Where(s => s.PaymentMethod == parsedMethod)
                .Include(s => s.SaleItems)
                    .ThenInclude(si => si.Product)
                .ToListAsync();
        ...
        public async Task<ActionResult<List<ProductSalesReportViewModel>>> GetTopSellingProducts(int topCount = 10)
        {
...
public async Task<ActionResult<Sale>> Create([FromBody] SaleCreateDto dto)
{
    ...
    var sale = await _salesService.CreateSaleAsync(dto.SaleItems, userId);
...
public async Task<ActionResult> Delete(int id)
{
    ...
    var deletedSale = new DeletedSale
    {
        ...
        SaleItems = sale.SaleItems.Select(i => new DeletedSaleItem
        {
    ...
    return Ok("Sale archived and deleted successfully.");
        ...
        {
            if (saleIds == null || !saleIds.Any()) return BadRequest("No sale IDs provided.");

            var salesToDelete = await _db.Sales
                .Where(s => saleIds.Contains(s.Id))
                .ToListAsync();

            if (!salesToDelete.Any()) return NotFound("No sales found for provided IDs.");
        ...
        public async Task<ActionResult<int>> DeleteByDateRange(DateTime start, DateTime end)
        {
            ...
            if (!salesToDelete.Any()) return NotFound("No sales found in range.");
